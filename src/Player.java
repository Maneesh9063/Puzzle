...
public class Player {
	...
	public boolean canPlace(int x, int y, PuzzlePiece piece) {
		if(grid.isValid(x, y))
		{
			...
			if(adjacentSide(x,y,PuzzlePiece.NORTH) == 0)
				...
				north = piece.getSide(PuzzlePiece.NORTH) + adjacentSide(x,y,PuzzlePiece.NORTH) ==0;
			//check east
			boolean east;
			if(adjacentSide(x,y,PuzzlePiece.EAST) == 0)
				east = true;
			else
				east = piece.getSide(PuzzlePiece.EAST) + adjacentSide(x,y,PuzzlePiece.EAST) ==0;
			//check west
			boolean south;
			if(adjacentSide(x,y,PuzzlePiece.SOUTH) == 0)
				south = true;
			else
				south = piece.getSide(PuzzlePiece.SOUTH) + adjacentSide(x,y,PuzzlePiece.SOUTH) ==0;
			//check south
			boolean west;
			if(adjacentSide(x,y,PuzzlePiece.WEST) == 0)
				west = true;
			else
				west = piece.getSide(PuzzlePiece.WEST) + adjacentSide(x,y,PuzzlePiece.WEST) ==0;
	...
	public int adjacentSide(int x, int y, int direction)
	{
		...
		{
			if(grid.isValid(x,y-1) && grid.isOccupied(x,y-1))
				return grid.getCell(x, y-1).getSide(PuzzlePiece.SOUTH);
			return 0;
		}
		if(direction == PuzzlePiece.EAST)
		{
			if(grid.isValid(x+1, y) && grid.isOccupied(x+1, y))
